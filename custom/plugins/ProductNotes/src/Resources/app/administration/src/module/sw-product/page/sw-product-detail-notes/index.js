...
Component.register('sw-product-detail-notes', {
    ...
    computed: {
        ...
        noteRepository() {
            return this.repositoryFactory.create('product_note');
        },
        ...
        noteColumns() {
            return [
                {
                    property: 'note',
                    label: this.$t('product-notes.detail.columnNote'),
                    rawData: true
                },
                {
                    property: 'solved',
                    label: this.$t('product-notes.detail.columnSolved'),
                    rawData: true
                },
                {
                    property: 'createdAt',
                    label: this.$t('product-notes.detail.columnCreatedAt'),
                    rawData: true
                }
            ];
        }
    ...
    methods: {
            ...
            this.noteRepository.delete(this.noteToDelete.id, Context.api)
                .then(() => {
                    this.createNotificationSuccess({
                        message: this.$t('product-notes.detail.messageDeleteSuccess')
                    });
                    this.loadNotes();
                })
                .catch(() => {
                    this.createNotificationError({
                        message: this.$t('product-notes.detail.messageError')
                    });
                })
                .finally(() => {
        ...
        onSaveNote() {
            this.noteRepository.save(this.currentNote, Context.api)
                .then(() => {
                    this.createNotificationSuccess({
                        message: this.$t('product-notes.detail.messageSaveSuccess')
                    });
