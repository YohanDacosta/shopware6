...
const bugger = (0, _debug.default)('babel-plugin-shopware-vite-meta-glob');
function viteMetaGlobBabelPlugin({
  ...
  const isGlobKey = propertyName => propertyName === 'glob';
  return {
    name: 'shopware-vite-meta-glob',
    visitor: {
      CallExpression(path, state) {
        const memberExpression = t.isMemberExpression(path.node.callee) && path.node.callee;
        if (!memberExpression) {
          return;
        }
        const args = path.node.arguments;
        const sourceFile = state.file.opts.filename;
        const propertyName = t.isIdentifier(memberExpression.property) && memberExpression.property.name;
        if (!sourceFile || !isGlobKey(propertyName) || !t.isMetaProperty(memberExpression.object)) {
          return;
        }
        if (!t.isStringLiteral(args[0])) {
          console.warn(`Did not transform ${sourceFile} because the first argument is not a single string pattern`);
          return;
        }
        ...
        if (args.length === 1) {
          ...
          const replacement = t.objectExpression(globPaths.map(globPath => t.objectProperty(t.stringLiteral(globPath), asts.glob(globPath))));
        ...
        if (args.length === 2) {
          ...
          if (eagerOption[0].value.value) {
            ...
            const variable = t.variableDeclaration('const', [t.variableDeclarator(t.identifier('__glob__context__'), t.objectExpression(globPaths.map((globPath, idx) => t.objectProperty(t.stringLiteral(globPath), identifiers[idx]))))]);
            const returnStatement = t.returnStatement(t.identifier('__glob__context__'));
